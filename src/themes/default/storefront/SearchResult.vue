...
const { hooks: searchTopHooks, fetchHooks: fetchSearchTopHooks } = useStorefrontIframeHooks('storefront_search_result_top');
const { hooks: searchBottomHooks, fetchHooks: fetchSearchBottomHooks } = useStorefrontIframeHooks('storefront_search_result_bottom');
...
const breadcrumbItems = computed(() => {
    ...
    return [
        { name: t('Home'), url: '/' },
        { name: `${t('Search results for')} "${keyword}"` },
    ];
});

onMounted(async () => {
    fetchSearchTopHooks();
    fetchSearchBottomHooks();

    suppressQuerySync.value = true;
    hydrateFromRoute(route);

    await listingStore.fetchProductsByKeyword(route.query.keyword, getUrlParams(['keyword']));

    suppressQuerySync.value = false;

    loaded.value = true;

    document.title = route.query.keyword + ' - Search';
});
...
onBeforeRouteUpdate(async (to, from, next) => {
    ...
    const { keyword, ...rest } = to.query;
    await listingStore.fetchProductsByKeyword(
        keyword,
        { ...rest, sort: rest.sort || 'sorting' }
    );
    ...
    <div>
        ...
        <template v-else>
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 pb-16">
                ...
                <!-- Storefront hook: search result top -->
                <StorefrontIframeHook
                    v-for="hook in searchTopHooks" :key="hook.id"
                    :hook="hook" hook-point="storefront_search_result_top"
                    :query-params="{ keyword: route.query.keyword, lang: locale }"
                />
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <div v-else class="flex flex-col items-center justify-center py-24 text-center">
                            ...
                            <p class="!mb-0 mt-2 text-sm text-[#8a8274]">
                                {{ $t('Try a different search term or browse our categories.') }}
                            </p>
                            <button
            ...
            <!-- Storefront hook: search result bottom -->
            <StorefrontIframeHook
                v-for="hook in searchBottomHooks" :key="hook.id"
                :hook="hook" hook-point="storefront_search_result_bottom"
                :query-params="{ keyword: route.query.keyword, lang: locale }"
            />
        </template>
