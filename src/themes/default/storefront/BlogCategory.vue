...
onMounted(async () => {
    getPaginatedArticles({ sort: 'new', category: route.params.slug });
});

onBeforeRouteUpdate(async (to, from) => {
    if (to.params.slug !== from.params.slug || to.query.page !== from.query.page) {
        getPaginatedArticles({ ...to.query, category: to.params.slug });
    }
});
...
const getPaginatedArticles = async (obj) => {
    ...
    document.title = translateItemField(categoryInfo.value, 'meta_title', locale.value) || zucConfig.store_name;
    const metaDesc = document.querySelector('meta[name="description"]');
    if (metaDesc) metaDesc.content = translateItemField(categoryInfo.value, 'meta_description', locale.value) || '';
}
    ...
    <div class="bg-[#f5f0e8]">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10 sm:py-14">
            ...
            <div class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-10 sm:mb-14">
                <div>
                    <h1 class="text-3xl sm:text-4xl tracking-tight text-[#2d2d2d] uppercase !mb-0" style="font-family: var(--font-display)" v-if="Object.keys(categoryInfo).length > 0">
                        {{ translateItemField(categoryInfo, 'name', $i18n.locale) }}
                    </h1>
