...
const submitForm = async() => {
    try {
        ...
        redirectTo(`/return-exchange/${verified.ref}`);
    } catch (error) {
        ...
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-14">
            <div class="max-w-md mx-auto">
                <div class="text-center mb-8">
                    <h1 class="text-2xl sm:text-3xl font-semibold text-gray-900 tracking-tight !mb-0">{{ $t('Returns & Exchanges') }}</h1>
                    <p class="text-sm text-gray-500 mt-1.5 !mb-0">{{ $t('To make a return or exchange, fill out the form below. We will process your request as quickly as possible. If we need more info to complete the request, we will contact you.') }}</p>
                </div>
                <div class="border border-gray-200 rounded-xl p-6 sm:p-8">
                    <form @submit.prevent="submitForm" class="space-y-5">
                        ...
                        <div>
                            <button type="submit" class="w-full flex justify-center bg-gray-900 hover:bg-gray-800 text-white rounded-xl px-6 py-2.5 font-medium text-sm transition-colors">{{ $t('Next') }}</button>
                        </div>
                    </form>
                </div>
            </div>
