...
const { hooks: manufacturerTopHooks, fetchHooks: fetchManufacturerTopHooks } = useStorefrontIframeHooks('storefront_manufacturer_top');
const { hooks: manufacturerBottomHooks, fetchHooks: fetchManufacturerBottomHooks } = useStorefrontIframeHooks('storefront_manufacturer_bottom');
...
onMounted(async () => {
    fetchManufacturerTopHooks();
    fetchManufacturerBottomHooks();
...
onBeforeRouteUpdate(async (to, from, next) => {
    suppressQuerySync.value = true;
    hydrateFromRoute(to);
    await listingStore.fetchProductsByManufacturer(
        to.params.slug,
...
watch(() => sortBy.value, (newSort, oldSort) => {
    if (suppressQuerySync.value) return;
    ...
    <div>
        ...
        <template v-else>
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 pb-16">
                ...
                <!-- Storefront hook: manufacturer top -->
                <StorefrontIframeHook
                    v-for="hook in manufacturerTopHooks" :key="hook.id"
                    :hook="hook" hook-point="storefront_manufacturer_top"
                    :query-params="{ manufacturer_id: listingStore.listingObject?.id, lang: locale }"
                />

                <div class="mb-8">
                    <h1 class="text-4xl sm:text-5xl tracking-wide text-[#2d2d2d]" style="font-family: var(--font-display)">{{ listingComputed.objectTranslation?.name }}</h1>
                    <div class="w-12 h-0.5 bg-[#c8a45c] mt-3"></div>
                    <p v-if="listingComputed.objectTranslation?.description" class="!mb-0 mt-4 text-sm text-[#8a8274] max-w-2xl leading-relaxed" v-html="listingComputed.objectTranslation.description"></p>
                </div>

                <div class="flex items-center justify-between mb-6 pb-5 border-b border-[#e0d8c8]">
                    <div class="flex items-center gap-4">
                        <button v-if="hasFilters" type="button" class="lg:hidden inline-flex items-center gap-2 text-sm font-medium text-[#2d2d2d] hover:text-[#c8a45c] transition-colors" @click="mobileFiltersOpen = true">
                            <AdjustmentsHorizontalIcon class="size-5" />
                            {{ $t('Filters') }}
                            <span v-if="selectedFilters.length > 0" class="inline-flex items-center justify-center h-5 min-w-5 px-1 rounded-full bg-[#c8a45c] text-[#1a1a1a] text-xs font-medium">{{ selectedFilters.length }}</span>
                        </button>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <select id="sort-by" v-model="sortBy" class="text-sm border-[#e0d8c8] rounded-lg py-2 pl-3 pr-8 bg-white text-[#2d2d2d] focus:outline-none focus:ring-2 focus:ring-[#c8a45c]/20 focus:border-[#c8a45c] transition-colors">
                            <option v-for="item in sortByItems" :key="item.field" :value="item.field">{{ item.name }}</option>
                        </select>
                    </div>
                </div>

                <SelectedFiltersBar :filters="listingStore.filters" :selected-filters="selectedFilters" class="mb-6" />
                ...
                <div class="flex gap-8">
                    ...
                    <div class="flex-1 min-w-0">
                        <template v-if="+listingStore.paginationInfo.total > 0">
                            ...
                            <nav v-if="listingStore.paginationLinks && listingStore.paginationLinks.length > 3" class="mt-12 flex items-center justify-center gap-1">
                                <template v-for="(link, index) in listingStore.paginationLinks" :key="index">
                                    <LocalizedLink v-if="link.url" :to="{ path: `/manufacturer/${$route.params.slug}`, query: { ...getUrlParams(['page']), page: getUrlParam(link.url, 'page') } }" :class="['inline-flex items-center justify-center min-w-[40px] h-10 px-3 text-sm rounded-lg transition-colors', link.active ? 'bg-[#c8a45c] text-white font-medium' : 'text-[#8a8274] hover:bg-[#c8a45c]/10']">
                                        <span v-html="link.label"></span>
                                    </LocalizedLink>
                                    <span v-else :class="['inline-flex items-center justify-center min-w-[40px] h-10 px-3 text-sm rounded-lg', link.active ? 'bg-[#c8a45c] text-white font-medium' : 'text-[#e0d8c8] cursor-default']">
                                        <span v-html="link.label"></span>
                                    </span>
                                </template>
                            </nav>
                        ...
                        <div v-else class="flex flex-col items-center justify-center py-24 text-center">
                            ...
                            <p class="!mb-0 text-lg text-[#2d2d2d]" style="font-family: var(--font-display)">{{ $t('No products found') }}</p>
                            <p class="!mb-0 mt-2 text-sm text-[#8a8274]">{{ $t('Try adjusting your filters or browse other manufacturers.') }}</p>
                            <button v-if="selectedFilters.length > 0" type="button" class="mt-6 inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-[#2d2d2d] bg-white border border-[#e0d8c8] rounded-lg hover:bg-[#f5f0e8] transition-colors" @click="selectedFilters = []">
                                {{ $t('Clear all filters') }}
                            </button>
                        </div>
                    </div>
            ...
            <!-- Storefront hook: manufacturer bottom -->
            <StorefrontIframeHook
                v-for="hook in manufacturerBottomHooks" :key="hook.id"
                :hook="hook" hook-point="storefront_manufacturer_bottom"
                :query-params="{ manufacturer_id: listingStore.listingObject?.id, lang: locale }"
            />
        </template>
                    ...
                    <TransitionChild as="template" enter="transition ease-in-out duration-300 transform" enter-from="-translate-x-full" enter-to="translate-x-0" leave="transition ease-in-out duration-300 transform" leave-from="translate-x-0" leave-to="-translate-x-full">
                        <DialogPanel class="relative flex w-full max-w-sm flex-col bg-white shadow-xl">
                            <div class="flex items-center justify-between px-5 py-4 border-b border-[#e0d8c8]">
                                ...
                                <button type="button" class="flex items-center justify-center size-8 rounded-full text-[#8a8274] hover:text-[#2d2d2d] hover:bg-[#f5f0e8] transition-colors" @click="mobileFiltersOpen = false">
                            ...
                            <div class="border-t border-[#e0d8c8] px-5 py-4">
                                <button type="button" class="w-full rounded-lg bg-[#1a1a1a] py-3 text-sm font-medium text-white hover:bg-[#333] transition-colors" @click="mobileFiltersOpen = false">
                                    {{ $t('Show results') }}
                                </button>
                            </div>
                        </DialogPanel>
